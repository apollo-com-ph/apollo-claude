...
- `bin/recommended-statusline.sh` — statusline script. Reads stdin JSON, fetches OAuth usage from Anthropic API (cached 10 min, flock-protected), outputs `[Model]XX%/$Y.YY (remaining% reset) parent/project`. Also writes `/tmp/statusline.json`.
