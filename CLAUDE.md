...
- `recommended-settings.json` — example `~/.claude/settings.json` with suggested permission defaults (allow/deny list, PreToolUse hook, model). Not installed automatically; developers copy or reference it manually. The deny list covers both Bash and Write/Edit/Read tools, protecting Claude infrastructure files (`~/.claude/`), shell dotfiles, SSH keys, and common credential stores.
- `safe-bash-patterns.json` — remote patterns file for `safe-bash-hook` (version 3). Contains the overridable tier of deny/allow patterns (49 deny patterns). Covers destructive git ops and gh api verbs, network transfer tools, additional credential reads (GPG, GitHub CLI, `.netrc`, `.git-credentials`), alternative credential readers (`xxd`, `strings`, `base64`, `od`), supply chain operations (`npm publish`, `gh release create`), scripted exfiltration, service management, and container security. Also includes `allow` rules for safe variants like `git push --force-with-lease`. Fetched hourly in the background by the hook; served via `raw.githubusercontent.com`.
