...
- `safe-bash-patterns.json` — remote patterns file for `safe-bash-hook`. Contains the overridable tier of deny/allow patterns (44 deny patterns). Covers destructive git ops and gh api verbs, network transfer tools, additional credential reads (GPG, GitHub CLI, `.netrc`, `.git-credentials`), scripted exfiltration, service management, and container security. Fetched hourly in the background by the hook; served via `raw.githubusercontent.com`.
...
- `hooks/safe-bash/` — Rust source for the `safe-bash-hook` binary. Two-tier pattern model: 42 hardcoded patterns (always enforced, cannot be overridden) + remote config patterns from `safe-bash-patterns.json` (blocked by default, overridable via `allow` rules). Exits 0 (allow) or 2 (block). Cross-compiled for Linux amd64/arm64 and macOS Intel/Apple Silicon. See `hooks/safe-bash/build.sh` for release builds.
