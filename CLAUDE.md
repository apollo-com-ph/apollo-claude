...
- `hooks/safe-bash/` — Rust source for the `safe-bash-hook` binary. Two-tier pattern model: 35 hardcoded patterns (always enforced, cannot be overridden) + remote config patterns from `safe-bash-patterns.json` (blocked by default, overridable via `allow` rules). Exits 0 (allow) or 2 (block). Cross-compiled for Linux amd64/arm64 and macOS Intel/Apple Silicon. See `hooks/safe-bash/build.sh` for release builds.
